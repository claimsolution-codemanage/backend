...
export const payCasePaymentSchedule = async (req, res) => {
    try {
        ...
        const paymentDetail = await CasePaymentInfo.findById(casePaymentDetailId).populate("clientId", "fullName mobileNo email profile.address profile.pinCode profile.state profile.city profile.panNo branchId");
        ...
        if (paymentDetail.gstPercent > 0 && paymentDetail.gstOption && !paymentDetail?.advocateMode) {
            await generateEmiInvoice({
                ...
                branchId: paymentDetail?.clientId?.branchId,
                client: {
